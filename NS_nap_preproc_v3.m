...
params.resampleICA        = 256;  % Resample the data for ICA saves computation time (0 = no resampling)
...
params.badchanmaxICs      = 1;   % Number of ICs to consider when looking for bad channels (between 0=none and 1=all)
...
%% Integrate wonambi annotations into EEG structure

% Sleep scoring
sleepscores = readtable([inpath, 'scoring\DA_fullNAP_RAW_sleepscores.csv']); % import sleep scoring output from Wonambi

myeegstg = scorecsv2mat(sleepscores, myeeg_raw);                       % Load scoring
myeeg_raw.preproc.stages = myeegstg;                   % Store sleep stage vector into EEG structure (before selection/resampling!)

% Artefact detection
eventscores = readtable([inpath, 'scoring\DA_fullNAP_RAW_sleepevents.csv']); % import event output from Wonambi

[ep_evt_str, ep_evt_end, ep_evt_szs] = ...
    eventcsv2mat(eventscores, myeeg_raw.srate, 'mov');                           % Load scoring
excpt = [ep_evt_str(:), ep_evt_end(:)];

% remove same segments from scoring:
incpt = true(1, myeeg_raw.pnts);
for i = 1:size(excpt,1)
    incpt(excpt(i,1):excpt(i,2)) = false;
end
...
% Exclude detected artefacts
myeeg = pop_select(myeeg, 'nopoint' , excpt);
myeeg.preproc.stages = myeeg_raw.preproc.stages(incpt);
% Downsampling
factor = myeeg_raw.srate / params.newsr;
myeeg = pop_resample(myeeg, params.newsr);                            
myeeg.preproc.stages = myeeg.preproc.stages(1:factor:end);
% Safety check
diff_pts = length(myeeg.preproc.stages) - myeeg.pnts;

% Set maximum allowed difference (1 second worth of data points)
max_allowed_diff = params.newsr;

if abs(diff_pts) > max_allowed_diff
    error('Mismatch too large (%d points), exceeds maximum allowed (%d points). Check preprocessing steps!', abs(diff_pts), max_allowed_diff);
else
    if diff_pts > 0
        % Stages vector is longer, trim excess points
        myeeg.preproc.stages = myeeg.preproc.stages(1:end-diff_pts);
    elseif diff_pts < 0
        % Stages vector is shorter, pad with last value
        myeeg.preproc.stages(end+1:end+abs(diff_pts)) = myeeg.preproc.stages(end);
    end
end
% Confirm final alignment
assert(length(myeeg.preproc.stages) == myeeg.pnts, 'Final alignment failed.');
