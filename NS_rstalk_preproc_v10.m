...
myeeg_rst = eeg_checkset(myeeg_raw);
sr = myeeg_rst.srate;

% Set 30s events to homogeneise processing with nap data
maxep = floor(myeeg_rst.pnts / (30 * sr)); % Number of 30s epochs
...
myeeg_rst.event = myevents;
myeeg_rst = eeg_checkset(myeeg_rst);

%% Preprocessing 
runtime = datetime;

% Set included EEG channels
myeeg_in = pop_select(myeeg_rst, 'channel', params.inchans);  
...
% Epoch & save data
pop_saveset(myeeg_out, 'filename', 'DA_restTalk_PREP.set', 'filepath', myoutdata, 'version','7.3');

myeeg_epRs = pop_epoch(myeeg_out, {'myep'}, [0  30], 'epochinfo', 'yes');
pop_saveset(myeeg_epRs, 'filename', 'DA_restTalkEp_PREP.set', 'filepath', myoutdata, 'version','7.3');
