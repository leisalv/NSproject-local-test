function EEG = zapclean(EEG, params, outpath)
%% EEG = zapclean(EEG,params,outpath)
%
% Clean data using zapline-plus and save report for each epoch
% Klug and Kloosterman, Human Brain Mapping 2022

srate = EEG.srate;
%chans = setxor(params.evaluationChannels, params.recreference);
    
signal = EEG.data(:,:)';
[cleand, ~, summary] = clean_data_with_zapline_plus(signal, srate, 'noisefreqs',50,...
  'winSizeCompleteSpectrum',floor(length(signal)/srate/8));
