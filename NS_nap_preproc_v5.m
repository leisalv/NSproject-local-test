...
%     myeeg_hp = pop_eegfiltnew(myeeg_in, params.highPassFilter, [], [], 0, [], 0);  % High pass filter
% end
% 
% % Downsampling
% disp('>> Downsample');
...
myeeg_zl = myzapcleaner(myeeg_in, myoutreps);
...
% Detect Artifacted data
disp('>> Artifact Rejection');
myeeg_clean = pop_clean_rawdata(myeeg_zl);
myeeg_clean = pop_saveset(myeeg_clean, 'filename', 'myeeg_clean.set', 'filepath', myoutdata);
[myeeg_cl, ~, myeeg_clNoRmv] = clean_artifacts(myeeg_zl, 'ChannelCriterion', 'off', 'LineNoiseCriterion','off', 'BurstCriterion', 20, 'BurstRejection', 'on'); % IF MANUAL INPUT: Select ASR without reconstruction + bad data segment detection [keeping default params]
vis_artifacts(myeeg_cl, myeeg_zl);
