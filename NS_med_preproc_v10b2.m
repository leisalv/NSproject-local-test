...
myeeg_nsRsEnd = pop_select(myeeg_raw, 'point',[med_end myeeg_raw.pnts]);
myeeg_nsRsEnd = eeg_checkset(myeeg_nsRsEnd);

% Set 30s events to homogeneise processing with nap data
maxep = floor(myeeg_nsRsEnd.pnts / (30 * sr)); % Number of 30s epochs
...
myeeg_in = pop_select(myeeg_nsRsEnd, 'channel', params.inchans);  
...
% Epoch & save data
pop_saveset(myeeg_out, 'filename', 'DA_medRsEnd_PREP.set', 'filepath', myoutdata, 'version','7.3');

myeeg_epMed = pop_epoch(myeeg_out, {'myep'}, [0 30], 'epochinfo', 'yes');
pop_saveset(myeeg_epMed, 'filename', 'DA_medRsEndEp_PREP.set', 'filepath', myoutdata, 'version','7.3');
