...
%%%%%% STEP 1: CESSATION SESSION (NAP + WAKE RS + NS) DATA PREPROCESSING SCRIPT
...
%% Select data within EEG structure

% Define NS section within meditation session 
% (10min rs - 90min NS - 10min rs)

medstart_tp = 10*60*recsr; %10mins * 60secs * Hz
medend_tp = myeeg_raw.pnts - 10*60*recsr; %end - (10mins * 60secs * Hz)
medtp = [medstart_tp medend_tp];
nomedtp = [1 medstart_tp; medend_tp myeeg_raw.pnts];

% Artefact detection
eventscores = readtable([inpath, 'scoring\DA_meditation_RAW_events.csv']); % import sleep scoring output from Wonambi

[ep_evt_str, ep_evt_end, ep_evt_szs] = ...
    eventcsv2mat(eventscores, recsr, 'mov');                           % Load scoring

eventtp = [ep_evt_str(:), ep_evt_end(:)];
exctp = [nomedtp; eventtp];
...
myeeg = pop_select(myeeg, 'nopoint' , exctp);                       % Exclude visually detected movement artefacts
...
pop_saveset(myeeg, 'filename', 'DA_meditation_PREP.set', 'filepath', myoutdata, 'version','7.3');
close all;
