...
% Load nap data
myeeg_raw = pop_loadset([inpath, 'raw\awkRest\DA_restful_wake_RAW.set']);
...
% % Sleep scoring
% sleepscores = readtable([inpath, 'scoring\DA_meditation_RAW_events.csv']); % import sleep scoring output from Wonambi
% 
% [ep_stg, ep_str, ep_end, ep_szs] = ...
%     scorecsv2mat(sleepscores, recsr);                           % Load scoring
...
%         end; clear cstg;
%     end
%     count=count+30*recsr;
% end

% Artefact detection
eventscores = readtable([inpath, 'scoring\DA_restful_wake_RAW_events.csv']); % import event scoring output from Wonambi
