function mybadchansrep(badchans, params, myeeg, outpath)
%% report_badchannels(badchannels, badreport, chanlocs, outpath)
%
% Report for bad channels / adapted to non-epoched data
...
if exist(figname, 'file')
    % If the file exists, modify the name to avoid overwriting
    figname = fullfile(outpath, 'Rep_BadChans_TopoICA.png');
end

fax=figure('units','normalized','outerposition',[0.1 0.10 0.8 0.8]);

badsign = sort(unique([badchans.all_marked, badchans.bc_ica]));
chanlocs = myeeg.chanlocs(params.evaluationChannels);
map_badsign = zeros(numel(chanlocs),1);
map_badsign(badsign==1) = 1;

title('Bad electrodes'); hold on;
prc_badsign = (numel(badsign)./numel(chanlocs)).*100;
