...
params.lowPassFilter      = 100;  % Low-pass filter for ICA (0 = no filter)
...
% myeeg_in = pop_select(myeeg_scored, 'channel', inc_chans);  
...
if params.highPassFilter>0  % High pass filter - comment out if doing highpass filtering during ASR
    [myeeg_hp, myeeg_mir] = myhpfilt(myeeg_scored, params.highPassFilter, params.mirrorWindow); 
end
...
myeeg_asr = pop_clean_rawdata(myeeg_zl, 'FlatlineCriterion','off','ChannelCriterion','off','LineNoiseCriterion','off','Highpass',[] ,'BurstCriterion',20,'WindowCriterion',0.25,'BurstRejection','on','Distance','Euclidian','WindowCriterionTolerances',[-Inf 7] );
