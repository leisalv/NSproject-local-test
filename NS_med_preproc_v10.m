...
myeeg_ns = pop_select(myeeg_raw, 'point',[med_start med_end]);
myeeg_ns = eeg_checkset(myeeg_ns);

% Set 30s events to homogeneise processing with nap data
maxep = floor(myeeg_ns.pnts / (30 * sr)); % Number of 30s epochs
...
myeeg_in = pop_select(myeeg_ns, 'channel', params.inchans);  
...
% Epoch & save data
pop_saveset(myeeg_out, 'filename', 'DA_meditation_PREP.set', 'filepath', myoutdata, 'version','7.3');

myeeg_epMed = pop_epoch(myeeg_out, {'myep'}, [0 30], 'epochinfo', 'yes');
pop_saveset(myeeg_epMed, 'filename', 'DA_medEp_PREP.set', 'filepath', myoutdata, 'version','7.3');
